...
CSSTHEMEFILES = css/${CSSTHEME}/jquery.mobile.theme.css
...
cssmin: init css
	...
	@@java -jar build/yuicompressor-2.4.4.jar --type css ${OUTPUT}/${CSSCORE} >> ${OUTPUT}/${CSSCOREMIN}
...
nightly: pull zip
	...
	@@find ${VER} -type f -name '*.html' -exec sed -i 's|href="css/${CSSTHEME}/"|href="${NIGHTLY_WEBPATH}/${DIR}.min.css"|g' {} \;
	...
	@@cp -R css/${CSSTHEME}/images ${OUTPUT}
...
deploy: zip
	...
	@@find ${VER} -type f -name '*.html' -exec sed -i "" -e 's|href="css/${CSSTHEME}/"|href="http://code.jquery.com/mobile/${VER}/${DIR}.min.css"|g' {} \;
