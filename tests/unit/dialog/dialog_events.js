...
(function($){
	...
	asyncTest( "dialog hash is added when the dialog is opened and removed when closed", function(){
		...
		setTimeout(function(){
			...
			ok($('#foo-dialog').find( ":jqmData(role=header)" ).hasClass('ui-bar-b'), 'Expected header theme ui-bar-b');
			ok($('#foo-dialog').find( ":jqmData(role=content)" ).hasClass('ui-body-b'), 'Expected content theme ui-body-b');
