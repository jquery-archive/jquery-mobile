 ...
 */
(function($){
    var changePageFn = $.mobile.changePage, originalTitle = document.title, siteDirectory = location.pathname.replace(/[^\/]+$/, "");
    ...
    asyncTest("forms with data attribute ajax set to false will not call changePage", function(){
        ...
        $.testHelper.sequence([ // avoid initial page load triggering changePage early
function(){
    ...
    asyncTest("forms with data attribute ajax not set or set to anything but false will call changepage", function(){
        var called = 0, newChangePage = function(){
            called++;
        };
...
function(){
    ...
    test("urlHistory is working properly", function(){
        ...
        //preload the stack
        $.mobile.urlHistory.stack[0] = {
            url: "foo",
            transition: "bar"
        };
        $.mobile.urlHistory.stack[1] = {
            url: "baz",
            transition: "shizam"
        };
        $.mobile.urlHistory.stack[2] = {
            url: "shizoo",
            transition: "shizaah"
        };
    ...
    asyncTest("Page events (beforeLoadingMsgIsShown, beforePageLoaded and afterPageLoaded) are called properly when the page successfully load", function(){
        expect(3);
        var blms = 0;
        var bpl = 0;
        var apl = 0;
        $(document).bind("afterLoadingMsgIsShown", function(){
            blms = 1;
        });
        $(document).bind("beforePageLoaded", function(){
            bpl = 1;
        });
        $(document).bind("afterPageLoaded", function(){
            apl = 1;
			$(document).unbind("afterPageLoaded");
        });
        $.testHelper.sequence([function(){
            $.mobile.loadPage("file.html");
        }, function(){
            equals(blms, 1, "beforeLoadingMsgIsShown was triggered");
            equals(bpl, 1, "beforePageLoaded was triggered");
            equals(apl, 1, "afterPageLoaded was triggered");
            $(document).unbind("beforeLoadingMsgIsShown");
            $(document).unbind("beforePageLoaded");
            start();
        }
],2000);
    });
	
	asyncTest("Page events (beforeLoadingMsgIsShown, beforePageLoaded, afterPageLoaded and failedLoadingPage) are called properly when the page failed to load", function(){
        expect(4);
        var blms = 0;
        var bpl = 0;
        var apl = 0;
        ...
        $(document).bind("afterPageLoaded", function(xhr){
            apl = 1;
            $(document).unbind("afterPageLoaded");
        });
		$(document).bind("failedLoadingPage", function(){
            flp = 1;
            $(document).unbind("failedLoadingPage");
        });
        $.testHelper.sequence([function(){
            $.mobile.loadPage("does-not-exist.html");
        }, function(){
            equals(blms, 1, "beforeLoadingMsgIsShown was triggered");
            equals(bpl, 1, "beforePageLoaded was triggered");
            equals(apl, 0, "afterPageLoaded was not triggered");
			equals(flp, 1, "failedLoadingPage was triggered");
            $(document).unbind("beforeLoadingMsgIsShown");
            $(document).unbind("beforePageLoaded");
            start();
        }
    ...
    asyncTest("last entry choosen amongst multiple identical url history stack entries on hash change", function(){
        ...
        $.testHelper.pageSequence([function(){
            ...
            $("#dup-history-dialog :jqmData(rel=back)").click();
        }, function(){
...
function(){
 ...
 function(){
    ...
    asyncTest("loading a relative file path after an embeded page works", function(){
        $.testHelper.pageSequence([ // transition second page
function(){
            $.testHelper.openPage("#relative-after-embeded-page-first");
        }, // transition second page
 function(){
            $("#relative-after-embeded-page-first a").click();
        }, // transition to the relative ajax loaded page
 function(){
            $("#relative-after-embeded-page-second a").click();
 ...
 function(){
    ...
    asyncTest("query param link from a dialog to itself should be a not add another dialog", function(){
        ...
        $.testHelper.pageSequence([ // open our test page
function(){
