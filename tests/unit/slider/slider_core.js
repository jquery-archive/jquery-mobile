...
(function($){
	...
	test( "slider elements in the keepNative set shouldn't be enhanced", function() {
		deepEqual( $("input.should-be-native").siblings(".ui-slider-contain").length, 0 );
	});
	...
	test( "data-highlight works properly", function() {
		...
		deepEqual( $highlighted.siblings( ".ui-slider-contain" ).find( ".ui-slider-bg" ).length, 1,
					"highlighted slider should have a div for the track bg" );
		deepEqual( $unhighlighted.siblings( ".ui-slider-contain" ).find( ".ui-slider-bg" ).length, 0,
