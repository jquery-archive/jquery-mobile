...
QUnit.test( "empty string value results defaults to slider min value", function( assert ) {
	...
	assert.deepEqual( slider.val( '' ).slider( 'refresh' ).val(), slider.attr( 'min' ), "val is equal to min attr" );
} );
...
QUnit.test( "labels without an id get an id", function( assert ) {
	...
	assert.equal( label.attr( "id" ), slider.attr( "id" ) + "-label", "the label id is based off the slider id" );
} );
...
QUnit.test( "slider tooltip & button values should match after input value changes", function( assert ) {
