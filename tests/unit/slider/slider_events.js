...
var keypressTest = function( assert, opts ) {
	...
	$.each( opts.keyCodes, function( i, elem ) {
		...
		val += opts.increment;
		assert.deepEqual( val, window.parseFloat( slider.val(), 10 ), "new value is " + opts.increment + " different" );
	} );
};
...
QUnit.test( "slider controls will create when inside a container that receives a 'create' event", function( assert ) {
	assert.ok( !$( "#enhancetest" ).appendTo( ".ui-page-active" ).find( ".ui-slider-track" ).length, "did not have enhancements applied" );
	assert.ok( $( "#enhancetest" ).enhance().find( ".ui-slider-track" ).length, "enhancements applied" );
} );
...
var assertLeftCSS = function( obj, opts ) {
	...
	if ( css.indexOf( "px" ) > -1 ) {
		...
		assert.ok( compare >= integerLeft - threshold && compare <= integerLeft + threshold, opts.message );
	} else {
...
QUnit.asyncTest( "drag should start only when clicked with left button", function( assert ) {
	...
	$.testHelper.detailedEventCascade( [
		...
		function( result ) {
			assert.deepEqual( result.slidestart.timedOut, false, "slider did emit 'slidestart' event upon 0 button press" );
			event = $.Event( "mousedown", { target: handle[ 0 ] } );
			event.which = 1;
			slider.trigger( event );
		},
		...
		function( result ) {
			assert.deepEqual( result.slidestart.timedOut, false, "slider did emit 'slidestart' event upon left button press" );
			event = $.Event( "mousedown", { target: handle[ 0 ] } );
		...
		function( result ) {
			assert.deepEqual( result.slidestart.timedOut, false, "slider did emit 'slidestart' event upon undefined button press" );
			event = $.Event( "mousedown", { target: handle[ 0 ] } );
			event.which = 2;
			slider.trigger( event );
		},
		...
		function( result ) {
			assert.deepEqual( result.slidestart.timedOut, true, "slider did not emit 'slidestart' event upon middle button press" );
			event = $.Event( "mousedown", { target: handle[ 0 ] } );
			event.which = 3;
			slider.trigger( event );
		},
		...
		function( result ) {
			assert.deepEqual( result.slidestart.timedOut, true, "slider did not emit 'slidestart' event upon right button press" );
			QUnit.start();
		}
	] );
} );

QUnit.asyncTest( "moving the slider triggers 'slidestart' and 'slidestop' events", function( assert ) {
...
if ( $.testHelper.versionTest( $.fn.jquery, function( l, r ) {
	...
	QUnit.test( "slider should detach event", function( assert ) {
		...
		assert.equal( getDocumentEventsLength( "vmouseup" ), ( vmouseupLength - 1 ), 'vmouseup event was removed' );
		assert.equal( getDocumentEventsLength( "vmousemove" ), ( vmousemoveLength - 1 ), 'vmousemove event was removed' );
	} );
}
} )( QUnit, jQuery );
